	...
	class Pager {
		...
		private function pageList(){
			...
			for($i=$inum; $i>=1; $i--){
				...
				if($page>=1)
					$linkPage.="<a style='color: #606261;padding: 4px 7px;' href='{$this->uri}page/{$page}'>{$page}</a>&nbsp;";

			}

			if($this->pageNum > 1)
				$linkPage.="<span style='color: #fff;display: inline-block;background: #f57f95;padding: 4px 7px;'>{$this->page}</span>&nbsp;";
		...
		private function goPage(){
    			if($this->pageNum > 1) {
				return '&nbsp;<input style="width:20px;height:17px !important;height:18px;border:1px solid #CCCCCC;" type="text" onkeydown="javascript:if(event.keyCode==13){var page=(this.value>'.$this->pageNum.')?'.$this->pageNum.':this.value;location=\''.$this->uri.'page/\'+page+\'\'}" value="'.$this->page.'"><input  type="button" value="GO" onclick="javascript:var page=(this.previousSibling.value>'.$this->pageNum.')?'.$this->pageNum.':this.previousSibling.value;location=\''.$this->uri.'page/\'+page+\'\'">&nbsp;';
			}
		}
